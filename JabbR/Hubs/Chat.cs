...
private async Task KickUserAsync(ChatUser targetUser, ChatRoom room, ChatUser callingUser, string reason)
{
    ...
    await OnRoomChanged(room);
        ...
        void INotificationService.CloseRoom(IEnumerable<ChatUser> users, ChatRoom room)
        {
            ...
            {
                Clients.User(user.Id).SendAsync("RoomClosed", room.Name).Wait();
            }

            // notify everyone to update their lobby
            OnRoomChanged(room).Wait();
        }
...
void INotificationService.UnCloseRoom(IEnumerable<ChatUser> users, ChatRoom room)
{
    ...
    foreach (var user in users)
    {
        Clients.User(user.Id).SendAsync("roomUnClosed", room.Name).GetAwaiter().GetResult();
    }

    // notify everyone to update their lobby
    OnRoomChanged(room).GetAwaiter().GetResult();
}
...
void INotificationService.Invite(ChatUser user, ChatUser targetUser, ChatRoom targetRoom)
{
    ...
    Clients.User(user.Id).SendAsync("sendInvite", user.Name, targetUser.Name, targetRoom.Name);
        ...
        {
            ChangeTopicAsync(user, room).GetAwaiter().GetResult();
        }

        private async Task ChangeTopicAsync(ChatUser user, ChatRoom room)
        {
...
void INotificationService.BroadcastMessage(ChatUser user, string messageText)
{
    ...
    foreach (var room in _repository.Rooms)
    {
        ...
        {
            string cookieValue;
            Context.Request.Cookies.TryGetValue(key, out cookieValue);
            return cookieValue != null ? Uri.UnescapeDataString(cookieValue) : null;
        }
